...
all: bin/champsim_mockingjay_4C_perceptron_next_line_instr
...
clean: 
	...
	$(RM) inc/ooo_cpu_modules.inc
	 ...
	 find replacement/mockingjay_4C -name \*.o -delete
	 find replacement/mockingjay_4C -name \*.d -delete
	 ...
	 find branch/perceptron -name \*.o -delete
	 find branch/perceptron -name \*.d -delete
	 find btb/basic_btb -name \*.o -delete
	 find btb/basic_btb -name \*.d -delete

bin/champsim_mockingjay_4C_perceptron_next_line_instr: $(patsubst %.cc,%.o,$(wildcard src/*.cc)) obj/repl_rreplacementDmockingjay_4C.a obj/pref_pprefetcherDno.a obj/repl_rreplacementDlru.a obj/pref_pprefetcherDnext_line_instr.a obj/pref_pprefetcherDip_stride.a obj/bpred_bbranchDperceptron.a obj/btb_bbtbDbasic_btb.a
	$(CXX) $(LDFLAGS) -o $@ $^ $(LDLIBS)

replacement/mockingjay_4C/%.o: CFLAGS += -Ireplacement/mockingjay_4C
replacement/mockingjay_4C/%.o: CXXFLAGS += -Ireplacement/mockingjay_4C
replacement/mockingjay_4C/%.o: CXXFLAGS +=  -Dinitialize_replacement=repl_rreplacementDmockingjay_4C_initialize -Dfind_victim=repl_rreplacementDmockingjay_4C_victim -Dupdate_replacement_state=repl_rreplacementDmockingjay_4C_update -Dreplacement_final_stats=repl_rreplacementDmockingjay_4C_final_stats
obj/repl_rreplacementDmockingjay_4C.a: $(patsubst %.cc,%.o,$(wildcard replacement/mockingjay_4C/*.cc)) $(patsubst %.c,%.o,$(wildcard replacement/mockingjay_4C/*.c))
	@mkdir -p $(dir $@)
	ar -rcs $@ $^
...
branch/perceptron/%.o: CFLAGS += -Ibranch/perceptron
branch/perceptron/%.o: CXXFLAGS += -Ibranch/perceptron
branch/perceptron/%.o: CXXFLAGS +=  -Dinitialize_branch_predictor=bpred_bbranchDperceptron_initialize -Dlast_branch_result=bpred_bbranchDperceptron_last_result -Dpredict_branch=bpred_bbranchDperceptron_predict
obj/bpred_bbranchDperceptron.a: $(patsubst %.cc,%.o,$(wildcard branch/perceptron/*.cc)) $(patsubst %.c,%.o,$(wildcard branch/perceptron/*.c))
	@mkdir -p $(dir $@)
	ar -rcs $@ $^
...
-include $(wildcard replacement/mockingjay_4C/*.d)
...
-include $(wildcard branch/perceptron/*.d)
