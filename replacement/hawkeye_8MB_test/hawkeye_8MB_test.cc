...
uint64_t paddr[LLC_SETS][LLC_WAYS];
uint32_t hcpu[LLC_SETS][LLC_WAYS];
...
uint16_t SetHit[LLC_SETS];
uint16_t cpu0_Hit[LLC_SETS];
uint16_t cpu1_Hit[LLC_SETS];
uint16_t cpu2_Hit[LLC_SETS];
uint16_t cpu3_Hit[LLC_SETS];
...
void CACHE::update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
	...
	if(hit && hcpu[set][way]!=cpu)
		cout<<"cpu doesn't match"<<endl;	
    ...
    //Ignore writebacks
     if (type == WRITEBACK){
		if(set==502){
			...
			cout<<TYPE_NAME<<" cpu="<<cpu<<" set="<<set<<" way="<<way<<" Paddr="<<full_addr<<" sig="<<signatures[set][way]<<" PC="<<ip<<" set_access="<<SetAccess[set]<<" victim_addr="<<victim_addr<<" instr_num="<<instr_num<<endl;
			for(uint32_t i=0; i<LLC_WAYS; i++)
			 	cout<<"  "<<i<<".sign:"<<paddr[set][i]<<" rrpv:"<<rrpv[set][i]<<" cpu:"<<hcpu[set][i];
			cout<<endl;
...
void CACHE::replacement_final_stats()
{
	...
	uint32_t total_hitt=0, total_cpu0_Hit=0, total_cpu1_Hit=0, total_cpu2_Hit=0, total_cpu3_Hit=0;
	for(uint32_t i=0; i<LLC_SETS; i++)
	{
		cout<<"set="<<i<<" Hit="<<SetHit[i]<<" cpu0_Hit="<<cpu0_Hit[i]<<" cpu1_Hit="<<cpu1_Hit[i]<<" cpu2_Hit="<<cpu2_Hit[i]<<" cpu3_Hit="<<cpu3_Hit[i]<<endl;
		total_hitt = total_hitt + SetHit[i];
		total_cpu0_Hit = total_cpu0_Hit + cpu0_Hit[i];
		total_cpu1_Hit = total_cpu1_Hit + cpu1_Hit[i];
		total_cpu2_Hit = total_cpu2_Hit + cpu2_Hit[i];
		total_cpu3_Hit = total_cpu3_Hit + cpu3_Hit[i];
 	}
	cout<< "total_hitt= "<<total_hitt<<endl;
	cout<< "total_cpu0_Hit= "<<total_cpu0_Hit<<endl;
	cout<< "total_cpu1_Hit= "<<total_cpu1_Hit<<endl;
	cout<< "total_cpu2_Hit= "<<total_cpu2_Hit<<endl;
	cout<< "total_cpu3_Hit= "<<total_cpu3_Hit<<endl;
    return;
}
