...
{	cout<<"LLC MockingJay Initialiaztion"<<endl;
	...
	cout<<"Sampled cache size:"<<sampled_cache.size()<<endl;
	// Each set has SAMPLED_CACHE_WAYS ways (should all be same)
}
...
uint32_t CACHE::find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
    ...
    uint64_t pc_signature = get_pc_signature(ip, false, type == PREFETCH, cpu);
...
void CACHE::update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
    ...
    ip = get_pc_signature(ip, hit, type == PREFETCH, cpu);
    ...
    if (is_sampled_set(set)) {
        ...
        for (int w = 0; w < SAMPLED_CACHE_WAYS; w++) {
            if (sampled_cache[sampled_cache_index][w].valid == false) {
                ...
                sampled_cache[sampled_cache_index][w].signature = ip;
    ...
    if (way < LLC_WAY) {
        if(!rdp.count(ip)) {
            ...
            if(rdp[ip] > MAX_RD) {
                ...
                etr[set][way] = rdp[ip] / GRANULARITY;
            }
        }
    }
}
