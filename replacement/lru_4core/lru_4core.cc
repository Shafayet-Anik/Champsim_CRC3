...
{	
	...
	if(llc_addr[set][way] != full_addr){
		llc_victim = llc_addr[set][way];
		llc_addr[set][way]= full_addr;
	}
...
if(set>=0){	
    ...
    // uncomment this line to see the LLC accesses
     ...
     cout << hex << "   full_addr:"<<setw(12)<< full_addr <<"   ip:" << ip <<"  victim_addr:"<< victim_addr <<"	 llc_victim:"<<llc_victim<< dec <<"  SetAccess:"<<SetAccess[set]<< endl;
}
*/
...
{
	//uint32_t total_hit_503=0;
	/*
	for(int i=0; i<=150; i++){
		cout<<"reuse_freq_distance["<<i<<"]:"<<reuse_freq503[i];
		total_hit_503= total_hit_503+reuse_freq503[i];
		cout<<" total:"<<total_hit_503<<endl;
	}
	cout<<"total_hit without WB**="<<total_hit_503<<endl;
	cout<<"WB_hit_**="<<WB_hit_503<<endl;
	total_hit_503 = total_hit_503+WB_hit_503;
	cout<<"Total_hit_***="<<total_hit_503<<endl;
	*/
